...
Returns the parsed JSON request body, or `nothing` when the body is empty or malformed, or when
the request's `Content-Type` is not JSON (`application/json` or `application/*+json`; a missing
header counts as not JSON). A cross-site page can send `text/plain` or no type without a CORS
preflight, so a body that does not declare itself JSON is not read as JSON (#327) -- and so
[`payload`](@ref) and `CSRFMiddleware`'s JSON token lookup, which read through this accessor, do
not see it either.
