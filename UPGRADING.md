...
> **Superseded by #176** (see the entry at the top of this file). Teardown now drains: it asks
> in-flight runs to stop, waits, and keeps the handle of anything it could not finish, so the
> workaround above is no longer the only option. `drain_timeout=0` reproduces the behaviour this
> paragraph describes.
