...
!!! warning "Decoded values can contain `/` and `..`"
    ...
    Nitro's own `staticfiles`/`spafiles` mounts are unaffected. A mount registers one prefix
    handler and enumerates the folder once, at mount time. Each request's decoded path is then
    looked up in that enumerated set and never joined onto the filesystem, so
    `..%2F..%2Fetc%2Fpasswd` decodes to a name that is simply not in the set.
