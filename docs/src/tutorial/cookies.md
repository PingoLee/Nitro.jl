...
Nitro provides a robust, high-performance cookie management system with native support for encryption and declarative data fetching via Extractors.
...
By default, Nitro sets `HttpOnly=true`, `Secure=true`, and `SameSite=Lax` for better security.
...
If a `secret_key` is configured, Nitro can automatically encrypt and decrypt your cookies using AES-256-GCM.
