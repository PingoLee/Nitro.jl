...
As your application grows you'll need to spread your application's logic across multiple files. Nitro provides some tools to help you do this while staying organized.
...
Both endpoints in this case will be tagged to the `math` tag and the `/multiply` endpoint will have an additional tag appended just to this endpoint called `multiplication`. These tags are used by Nitro when auto-generating the documentation to organize it by separating the endpoints into sections based off their tags. 
