...
<div>
  ...
  <p align="center">
    ...
    <!-- <a href='https://codecov.io/gh/NitroFramework/Nitro.jl'><img src='https://codecov.io/gh/NitroFramework/Nitro.jl/branch/master/graph/badge.svg' alt='Coverage Status' /></a> -->
  </p>
</div>
```
...
# Register the route and start the web server
serve(urlpatterns=[
    path("/greet", greet)
])
```
...
- Handlers can be imported from other modules and distributed across multiple files for better organization
- They are simple Julia functions that receive a `Request` (or `Stream`/`WebSocket`) and return a response
- Registration is centralized using `path()` and `urlpatterns()`
...
# Map them in urlpatterns
serve(urlpatterns=[
    path("/", request_handler),
    path("/stream", stream_handler; method="STREAM"),
    path("/ws", ws_handler; method="WEBSOCKET")
])
```
...
p1 = path("/greet", function()
    "hello world!"
end)

# A one-liner via do-syntax assigned to a path
p2 = path("/gruessen") do req 
    "Hallo Welt!"
end
...
# Register passing the function directly
p3 = path("/subtract/<float:a>/<float:b>", subtract)

# Serve them all
serve(urlpatterns=[p1, p2, p3])
```
...
### Request Handlers
Request handlers are used to handle HTTP requests. They are defined as simple functions and registered using the `path()` function. 

- Use `method="POST"` (or other HTTP methods) in `path()` to change the HTTP method. Defaults to `GET`.

### Stream Handlers
Stream handlers are used to stream data. They accept a `HTTP.Stream` object. To register one, pass `method="STREAM"` to `path()`.

### Websocket Handlers
Websocket handlers are used to handle websocket connections. They accept a `HTTP.WebSocket` object. To register one, pass `method="WEBSOCKET"` to `path()`.

</details>


## Django-Style Routing Syntax

Nitro.jl uses a centralized, Django-inspired routing system. Instead of sprinkling routing macros throughout your codebase, you define clear `path()` mappings and group them into `urlpatterns`.

```julia
# Define handler functions
function add(req::HTTP.Request, x::Int, y::Int)
    x + y
end

# Group routes into urlpatterns
api_routes = [
    path("/add/<int:x>/<int:y>", add)
]

# Register with the server
serve(urlpatterns=api_routes)
```

You can also include nested route modules using `include_routes()`.
...
Nitro comes with a built-in cron scheduling system that allows you to call endpoints and functions automatically when the cron expression matches the current time.
...
The cron parser in Nitro is based on the same specifications as the one used in Spring. You can find more information about this on the [Spring Cron Expressions](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/scheduling/support/CronExpression.html) page.
...
In addition, Nitro provides utility functions to manually start and stop cron jobs: `startcronjobs()` and `stopcronjobs()`. These functions can be used outside of a web server as well.
...
For scenarios where you need to handle higher amounts of traffic, you can run Nitro in a 
...
Nitro is equipped with several package extensions that enhance its plotting capabilities. These extensions make it easy to return plots directly from request handlers. All operations are performed in-memory using an IOBuffer and return a `HTTP.Response`
...
using Nitro
using Nitro: html # Bonito also exports html
...
Nitro provides a default logging format but allows you to customize the format using the `access_log` parameter. This functionality is available in both the `serve()` and `serveparallel()` functions.
...
Nitro ships with some prebuilt middleware functions so you can easily integrate bearer auth, rate limiting and CORS support to your app. You can add these at the application, router, or route level in your app through the `middleware` keyword.
...
The `Cors` middleware handles Cross-Origin Resource Sharing (CORS) for your API. It sets the right headers and responds to preflight OPTIONS requests, so browsers can safely call your endpoints from other domains. Just configure your policy with keyword arguments and Nitro will do the rest.
