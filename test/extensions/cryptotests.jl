...
@testset "NitroCryptoExt Tests" begin
    ...
    @testset "Basic Encrypt/Decrypt" begin
        ...
        encrypted = Nitro.Crypto.encrypt_payload(secret, payload)
        
        @test encrypted != payload
        @test !isempty(encrypted)
        
        decrypted = Nitro.Crypto.decrypt_payload(secret, encrypted)
    ...
    @testset "Integrity Check (Tampering)" begin
        ...
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload(secret, tampered)
    end

    @testset "Invalid Secret" begin
        encrypted = Nitro.Crypto.encrypt_payload(secret, payload)
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload("wrong-secret", encrypted)
    end

    @testset "Malformed Payloads" begin
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload(secret, "not-base64-!@#\$")
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload(secret, "abcd") # too short
    end

    @testset "Large Payloads" begin
        large_payload = "a" ^ 10_000
        encrypted = Nitro.Crypto.encrypt_payload(secret, large_payload)
        decrypted = Nitro.Crypto.decrypt_payload(secret, encrypted)
    ...
    @testset "Non-Determinism (IV Randomization)" begin
        ...
        enc1 = Nitro.Crypto.encrypt_payload(secret, payload)
        enc2 = Nitro.Crypto.encrypt_payload(secret, payload)
        @test enc1 != enc2
        @test Nitro.Crypto.decrypt_payload(secret, enc1) == payload
        @test Nitro.Crypto.decrypt_payload(secret, enc2) == payload
    end

    @testset "Unicode & Special Characters" begin
        unicode_payload = "🚀 Nitro.jl is fast! ⚡ (ñ, ü, 中文)"
        encrypted = Nitro.Crypto.encrypt_payload(secret, unicode_payload)
        decrypted = Nitro.Crypto.decrypt_payload(secret, encrypted)
    ...
    @testset "Empty Payload" begin
        empty_payload = ""
        encrypted = Nitro.Crypto.encrypt_payload(secret, empty_payload)
        decrypted = Nitro.Crypto.decrypt_payload(secret, encrypted)
    ...
    @testset "Base64URL Padding Symmetry" begin
        ...
        for len in 1:10
            p = "x" ^ len
            enc = Nitro.Crypto.encrypt_payload(secret, p)
            @test Nitro.Crypto.decrypt_payload(secret, enc) == p
        end
    end
    ...
    @testset "Key Derivation Consistency" begin
        ...
        enc1a = Nitro.Crypto.encrypt_payload(secret_consistent, payload1)
        enc1b = Nitro.Crypto.encrypt_payload(secret_consistent, payload1)
        
        # Both encrypt to different ciphertexts (different IVs) but both decrypt correctly
        @test enc1a != enc1b
        @test Nitro.Crypto.decrypt_payload(secret_consistent, enc1a) == payload1
        @test Nitro.Crypto.decrypt_payload(secret_consistent, enc1b) == payload1
        
        # Different payloads produce different ciphertexts
        enc2 = Nitro.Crypto.encrypt_payload(secret_consistent, payload2)
        @test enc2 != enc1a
        @test enc2 != enc1b
    end
    ...
    @testset "IV Uniqueness Protection" begin
        ...
        encrypted_values = [Nitro.Crypto.encrypt_payload(secret, payload_iv) for _ in 1:10]
        ...
        decrypted_values = [Nitro.Crypto.decrypt_payload(secret, enc) for enc in encrypted_values]
    ...
    @testset "Thread-Safe Concurrent Operations" begin
        ...
        Threads.@threads for i in 1:(n_threads * n_ops)
            enc = Nitro.Crypto.encrypt_payload(secret, payloads[i])
            results[i] = enc
        end
        ...
        Threads.@threads for i in 1:(n_threads * n_ops)
            decrypted[i] = Nitro.Crypto.decrypt_payload(secret, results[i])
        end
    ...
    @testset "IV Region Tampering Specific" begin
        ...
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload(secret, tampered_iv)
    end
    ...
    @testset "Auth Tag Tampering Detection" begin
        ...
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload(secret, tampered_tag)
    end
    ...
    @testset "Short Secret Key Handling" begin
        ...
        encrypted_short = Nitro.Crypto.encrypt_payload(short_key, payload_short)
        @test !isempty(encrypted_short)
        
        # Should decrypt correctly
        decrypted_short = Nitro.Crypto.decrypt_payload(short_key, encrypted_short)
        @test decrypted_short == payload_short
        
        # But wrong short key should fail
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload("wrong", encrypted_short)
    end
    ...
    @testset "Decryption State Consistency" begin
        ...
        encrypted_state = Nitro.Crypto.encrypt_payload(secret, payload_state)
        
        # Decrypt multiple times, should always succeed
        for _ in 1:5
            result = Nitro.Crypto.decrypt_payload(secret, encrypted_state)
        ...
        p1 = "first"
        p2 = "second"
        enc1 = Nitro.Crypto.encrypt_payload(secret, p1)
        enc2 = Nitro.Crypto.encrypt_payload(secret, p2)
        
        # Decrypt in mixed order
        @test Nitro.Crypto.decrypt_payload(secret, enc2) == p2
        @test Nitro.Crypto.decrypt_payload(secret, enc1) == p1
        @test Nitro.Crypto.decrypt_payload(secret, enc2) == p2
    end
    ...
    @testset "Multiple Secrets Isolation" begin
        ...
        enc_with_secret1 = Nitro.Crypto.encrypt_payload(secret1, payload_isolation)
        
        # Should decrypt with correct secret
        @test Nitro.Crypto.decrypt_payload(secret1, enc_with_secret1) == payload_isolation
        
        # Should NOT decrypt with wrong secret
        @test_throws Nitro.Errors.CookieError Nitro.Crypto.decrypt_payload(secret2, enc_with_secret1)
    end

    @testset "Null Byte Handling" begin
        # Test payloads containing null bytes
        payload_null = "data\0with\0nulls"
        encrypted_null = Nitro.Crypto.encrypt_payload(secret, payload_null)
        decrypted_null = Nitro.Crypto.decrypt_payload(secret, encrypted_null)
