...
using Dates 
using Suppressor
using Nitro; @oxidise # intentionally use the deprecated spelling to trigger the warning
...
route(["GET"], "/killserver", function (; context)
    terminate()
end)

route(["GET"], "/anonymous", function(; request, context)
    return "no args"
end)

route(["GET"], "/anyparams/{message}", function(req, message::Any; context)
    return message
end)
...
route(["GET"], "/testredirect", function(req)
...
route(["GET"], router("/middleware-error", middleware=[errormiddleware]), function ()
...
route(["GET"], "/undefinederror", function ()
    asdf
end)

route(["GET"], "/unsupported-struct", function ()
    return Book("mobdy dick", "blah")
end)

try 
    route(["GET"], "/mismatched-params/{a}/{b}", function (a,c; request, context)
        return "$a, $c"
    end)
catch e
    @test true 
...
route(["GET"], "/add/{a}/{b}", function (req, a::Int32, b::Int64)
    return a + b
end)

route(["GET"], "/divide/{a}/{b}", function (req, a, b; request, context)
    return parse(Float64, a) / parse(Float64, b)
end)

# path is missing function parameter
try 
    route(["GET"], "/mismatched-params/{a}/{b}", function (req, a,b,c)
        return "$a, $b, $c"
    end)
catch e
    @test true
...
try 
    route(["GET"], "/mismatched-params/{a}/{b}", function (req, a)
        return "$a, $b, $c"
    end)
catch e
    @test true 
...
route(["GET"], "/multiply/{a}/{b}", function(req, a::Float64, b::Float64)
    return a * b 
end)

route(["GET"], "/person", function(req)
    return Person("joe", 20)
end)

route(["GET"], "/text", function(req)
    return text(req)
end)

route(["GET"], "/binary", function(req)
    return binary(req)
end)

route(["GET"], "/json", function(req)
    return json(req)
end)

route(["GET"], "/person-json", function(req)
    return json(req, Person)
end)

route(["GET"], "/html", function(req)
    return html(""" 
        <!DOCTYPE html>
            <html>
            <body> 
...
route(["GET"], "/query", function(req)
    return queryparams(req)
end)

route(["POST"], "/post", function(req)
    return text(req)
end)

route(["PUT"], "/put", function(req)
    return "put"
end)

route(["PATCH"], "/patch", function(req)
    return "patch"
end)

route(["DELETE"], "/delete", function(req)
    return "delete"
end)
...
route(["GET"], "/fruit/{fruit}", function(req, fruit::Fruit; request, context)
  return fruit
end)

route(["GET"], "/date/{date}", function(req, date::Date)
  return date
end)

route(["GET"], "/datetime/{datetime}", function(req, datetime::DateTime)
  return datetime
end)

route(["GET"], "/complex/{complex}", function(req, complex::Complex{Float64})
  return complex
end)

route(["GET"], "/list/{list}", function(req, list::Vector{Float32})
    return list
end)

route(["GET"], "/dict/{dict}", function(req, dict::Dict{String, Any})
  return dict
end)

route(["GET"], "/tuple/{tuple}", function(req, tuple::Tuple{String, String})
  return tuple
end)

route(["GET"], "/union/{value}", function(req, value::Union{Bool, String})
  return value
end)

route(["GET"], "/boolean/{bool}", function(req, bool::Bool)
  return bool
end)

route(["GET"], "/struct/{student}", function(req, student::Student)
  return student
end)

route(["GET"], "/float/{float}", function (req, float::Float32)
  return float
end)
...
route(["GET"], "/math/add/{a}/{b}", function (req::HTTP.Request, a::Float64, b::Float64)
    return a + b
end)

# case 1: no middleware is defined at any level -> use global middleware
route(["GET"], router("/math/power/{a}/{b}"), function (req::HTTP.Request, a::Float64, b::Float64)
    return a ^ b
end)
...
route(["GET"], math("/cube/{a}", middleware=[]), function(req, a::Float64)
    return a * a * a
end)

# case 3: router-level is empty & route-level is defined
other = router("/math", middleware=[])
route(["GET"], other("/multiply/{a}/{b}", middleware=[middleware3]), function (req::HTTP.Request, a::Float64, b::Float64)
    return a * b
end)
# case 4 (both defined)
route(["GET"], math("/divide/{a}/{b}", middleware=[middleware4]), function(req::HTTP.Request, a::Float64, b::Float64)
    return a / b
end)

# case 5: only router level is defined
route(["GET"], math("/subtract/{a}/{b}"), function(req::HTTP.Request, a::Float64, b::Float64)
    return a - b
end)

# case 6: only route level middleware is defined
empty = router()
route(["GET"], empty("/math/square/{a}", middleware=[middleware3]), function(req, a::Float64)
    return a * a
end)

emptyrouter = router()
route(["GET"], router("emptyrouter"), function(req)
    return "emptyrouter"
end)

emptysubpath = router("/emptysubpath", tags=["empty"])
route(["GET"], emptysubpath("", middleware=[middleware1]), function(req)
    return "emptysubpath"
end)

# added another request hanlder for post requests on the same route
route(["POST"], emptysubpath(""), function(req)
