...
error_task = @async begin
    for revise in (:lazy, :eager)
        @test_throws "You must load Revise.jl before Nitro.jl" serve(port=PORT, host=HOST, show_errors=false, show_banner=false, access_log=nothing, revise=revise)
    end
end
...
Nitro.WAS_LOADED_AFTER_REVISE[] = true
...
Nitro.WAS_LOADED_AFTER_REVISE[] = false
