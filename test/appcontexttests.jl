...
route(["GET"], "/injected", function(req, ctx::Context{Person})
    return json(ctx.payload)
end)

route(["GET"], "/kwarg-only", function(req; context)
    return json(context)
end)

route(["GET"], "/both-kwargs", function(; request::Request, context::Person)
    return json(context)
end)

route(["GET"], "/method-only", function()
    return json(context())
end)
