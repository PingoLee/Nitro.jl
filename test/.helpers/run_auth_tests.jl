...
@testset "All Auth Module Tests" begin
  ...
  @testset "Static cross-compatibility: Django 4.2 PBKDF2 fixture" begin
    # Hash in Django 4.2+ wire format for password "testpassword123"
  ...
  @testset "Static cross-compatibility: Spring Security 6.x PBKDF2 fixture" begin
    # Hash in Spring Security 6.x wire format for password "testpassword123"
  ...
  @testset "Argon2 scaffolding" begin
    enc = Nitro.Auth.Argon2PasswordEncoder()
    @test enc.memory_cost == 65536
    @test enc.time_cost == 3
    @test enc.parallelism == 4

    @test_throws ErrorException encode(enc, "test")
    @test_throws ErrorException matches(enc, "test", "hash")
    @test_throws ErrorException upgrade_encoding(enc, "hash")

    @test_throws ArgumentError Nitro.Auth.Argon2PasswordEncoder(memory_cost=4)
    @test_throws ArgumentError Nitro.Auth.Argon2PasswordEncoder(time_cost=0)
    @test_throws ArgumentError Nitro.Auth.Argon2PasswordEncoder(parallelism=0)

    result = Nitro.Auth.parse_argon2_phc("\$argon2id\$v=19\$m=65536,t=3,p=4\$c2FsdA==\$aGFzaA==")
    @test result !== nothing
    @test result.variant == "id"
    @test result.version == 19
    @test result.memory_cost == 65536
    @test result.time_cost == 3
    @test result.parallelism == 4

    @test Nitro.Auth.parse_argon2_phc("not_argon2") === nothing
    @test Nitro.Auth.parse_argon2_phc("pbkdf2_sha256\$720000\$salt\$hash") === nothing
  end

end # All Auth Module Tests
...
@testset "Password encoders" begin
