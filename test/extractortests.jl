...
@testset "Api tests" begin 
    ...
    route(["GET"], "/headers", function(req, headers = Header(Sample, s -> s.limit > 5))
        return headers.payload
    end)

    route(["POST"], "/form", function(req, form::Form{Sample})
        return form.payload |> json
    end)

    route(["GET"], "/query", function(req, query::Query{Sample})
        return query.payload |> json
    end)

    route(["POST"], "/body/string", function(req, body::Body{String})
        return body.payload
    end)

    route(["POST"], "/body/float", function(req, body::Body{Float64})
        return body.payload
    end)

    route(["POST"], "/json", function(req, data = Json{PersonWithDefault}(s -> s.value < 10))
        return data.payload
    end)
 
    route(["POST"], "/protobuf", function(req, data::ProtoBuffer{MyMessage})
        return  protobuf(data.payload)
    end)

    route(["POST"], "/json/partial", function(req, p1::JsonFragment{PersonWithDefault}, p2::JsonFragment{PersonWithDefault})
        return json((p1=p1.payload, p2=p2.payload))
    end)

    route(["GET"], "/path/add/{a}/{b}", function(req, a::Int, path::Path{Parameters}, qparams::Query{Sample}, c::Nullable{Int}=23)
