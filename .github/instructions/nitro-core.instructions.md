...
- **The two response namespaces, and which one has what.** `Res` (`src/response.jl`) provides exactly **`json`, `status`, `send`, `file`, `redirect`** — nothing else. There is **no `Res.html` and no `Res.js`.** Markup and script responses come from the *top-level exported* constructors in `src/utilities/render.jl`: `html()`, `js()`, `xml()`, `css()`, plus `text()`, `binary()`, and their own `json()`/`file()`. Those four — **`html`, `js`, `xml`, `css`** — are the only markup/script sinks in the framework, so they are the only place unescaped user input can become an XSS vector. Escape before you reach them, always.
  - `json`, `file`, and `redirect` currently exist in **both** namespaces with divergent signatures and defaults (`Res.file` defaults to `disposition="attachment"`; `file()` in `render.jl` does not). Reconciling this is tracked in [#28](https://github.com/PingoLee/Nitro.jl/issues/28) — until it lands, name the namespace explicitly (`Res.file(...)` vs `Nitro.file(...)`) rather than relying on which one is in scope.
  - `json` is also a *request body parser* in `src/utilities/bodyparsers.jl` (`json(req)`, `json(req, T)`). Same name, opposite direction — read the argument type before assuming which one a call site means.
